...
*******************************************************************
    ...
    /**
     * This function will cleanup emails formatted like 'Name <email@domain.com>' into an array( 'name'=>'Name', 'email'=>'email@domain.com')
     * to avoid errors when sending to Mandrill
     *
     * @param $email
     * @return array
     */
    static function correctEmail($email) {
        ...
        foreach ($m as $p) {
            if (filter_var($p, FILTER_VALIDATE_EMAIL)) {
               return array('name'=>$name, 'email'=>$p);
            } else {
                $name .= $p." ";
            }
        }
	        ...
	        *************************/
			            ...
			            foreach ( (array) $tempheaders as $header ) {
				            ...
				            switch ( strtolower( $name ) ) {
					            ...
					            case 'bcc':
                                    ...
                                    // >> Mandrill Does not support multiple bcc BUT you it will send the emails directly to the user
					                $bcc = array_merge( (array) $bcc, explode( ',', $content ) );

                                    $bcc_emails[] = array('email' => $content);

                                    // If you wish to send it as 'bcc' anyway, you can set the type here
                                    //$bcc_emails[] = array('email' => $content, 'type' => 'bcc');

                                    // So you don't need to use the bcc_address.
	        ...
	        // Checking To: field
                ...
                foreach ( $message['to'] as $email ) {
                    ...
                    if ( is_array($email) ) {
                        foreach ($email as $m) {
                            $processed_to[]=self::correctEmail($m);
                        }
                    } else {
                        $processed_to[] = self::correctEmail($email);
                    }

                }

                // As I don't send them as Bcc, I don't send it to th to recipents as each recipient qill receive it's own email.
                if (sizeof($bcc_emails) > 0) {
                    $message['to'] = $bcc_emails;
                }
//                $message['to'] = array_merge($processed_to,$bcc_emails);
